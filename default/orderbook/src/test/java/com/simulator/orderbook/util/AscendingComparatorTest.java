...
public class AscendingComparatorTest extends BaseUnitTest{
	...
	public void testNonNullNonEqualValues(){
		int compareResult = new AscendingComparator<Sequenceable<Quote>, Quote>().compare(new Sequenceable<Quote>(createQuote(24.0, 0)), 
	...
	public void testNonNullEqualValuesWithReverseSequence(){
		...
		int compareResult = new AscendingComparator<Sequenceable<Quote>, Quote>().compare(seq2, 
				seq1);
	...
	public void testNonNullEqualValuesWithSameSequence(){
		Sequenceable<Quote> seq1 = new Sequenceable<Quote>(createQuote(24.0, 0));
		int compareResult = new AscendingComparator<Sequenceable<Quote>, Quote>().compare(seq1, 
				seq1);
	...
	public void testWithNullFirstValue(){
		try{
			new AscendingComparator<Sequenceable<Quote>, Quote>().compare(null, 
					new Sequenceable<Quote>(createQuote(22.0, 0)));
	...
	public void testWithNullSecondValue(){
		try{
			new AscendingComparator<Sequenceable<Quote>, Quote>().compare(new Sequenceable<Quote>(createQuote(22.0, 0)),
					null);
			Assert.fail("Should not reach this point");
		}catch(Exception e){
			e.printStackTrace();
